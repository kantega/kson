...
public class ParserPropertiesTest {
    ...
    final Gen<String> nonEmptyAlphanum =
      Arbitrary
        .arbNonEmptyList(elements(
          range(charEnumerator, 'a', 'z')
            .append(range(charEnumerator, 'A', 'Z'))
            .append(range(charEnumerator, '0', '9'))
            .toArray().array(Character[].class)))
        .map(list -> List.asString(list.toList()));

    final Gen<String> nonEmptyAlphanumWithspaceAndNL =
      Arbitrary
        .arbNonEmptyList(elements(
          range(charEnumerator, 'a', 'z')
            .append(range(charEnumerator, 'A', 'Z'))
            .append(range(charEnumerator, '0', '9'))
            .append(Stream.arrayStream('\n',' '))
            .toArray().array(Character[].class)))
        .map(list -> List.asString(list.toList()));
    ...
    final Gen<JsonValue> jStringGen =
      nonEmptyAlphanumWithspaceAndNL.map(JsonValues::jString);
